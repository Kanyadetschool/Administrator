// Populates each Top Readers row's avatar once StudentImageManager (from
// the studentImages.js module script) is available, retrying briefly
// instead of silently giving up if it isn't ready yet. See the call site
// in renderTopReaders for why this exists.
function renderTopReaderAvatarsWhenReady(topReaders, container, attemptsLeft = 20) {
    if (typeof StudentImageManager === 'undefined') {
        if (attemptsLeft <= 0) {
            console.error('Top Readers: StudentImageManager never became available — check that js/studentImages.js loaded. If this page was opened directly as a file:// URL, module scripts are blocked by CORS and need to be served over http(s) instead.');
            return;
        }
        setTimeout(() => renderTopReaderAvatarsWhenReady(topReaders, container, attemptsLeft - 1), 100);
        return;
    }

    topReaders.forEach(([studentId]) => {
        ...
        const imageContainer = row?.querySelector('.student-image-container');
        if (!imageContainer) return;
        StudentImageManager.renderStudentImage(student?.name || student?.fullName || 'Unknown', student?.grade, imageContainer);
    });
}
...
class AdvancedAnalyticsManager {
    ...
    renderTopReaders(issuances) {
                        ...
                        ${topReaders.map(([studentId, count], index) => {
                            ...
                            return `
                                <tr data-student-id="${studentId}">
                                    <td>${index + 1}</td>
                                    <td>
                                        <div class="top-reader-cell">
                                            <div class="student-image-container top-reader-avatar"></div>
        ...
        // Images render after the rows exist in the DOM, same pattern used
        // for student cards / issuance cards elsewhere in app.js: an empty
        // .student-image-container is put in the markup up front, then
        // StudentImageManager fills it in per row.
        //
        // Deliberately no 4th (status) argument here: renderStudentImage
        // appends an absolute-positioned status pill sized for the big
        // 76-100px profile-card avatars used elsewhere. On this table's
        // 32px inline avatar that pill would overflow the circle, and the
        // Grade column right next to it already shows the same info.
        //
        // StudentImageManager comes from studentImages.js, a type="module"
        // script — module scripts are supposed to finish executing before
        // DOMContentLoaded fires, but a single point-in-time check here
        // silently skipped every avatar if that wasn't true (or if the
        // module failed to load at all, e.g. this page opened via file://,
        // which blocks ES module imports with a CORS error and leaves
        // StudentImageManager undefined with no visible error). Retrying
        // for a couple seconds, with a real console message if it never
        // shows up, replaces that silent no-op with a debuggable one.
        renderTopReaderAvatarsWhenReady(topReaders, container);
    }
