...
// NOTE: mobile sidebar open/close (toggle button, backdrop click, outside
// click, Escape, nav-link close) is owned entirely by the inline script
// near the bottom of library.html — it already does all of this against
// the real #sidebar / .sidebar-backdrop elements in the markup. A duplicate
// implementation used to live here too: it registered a second click
// listener on the same #sidebarToggle button and created its own extra
// .sidebar-backdrop element. Every toggle click then fired both handlers,
// which toggled .sidebar's "show" class twice (net no-op — sidebar never
// visibly opened) while showing one or both backdrop elements. That's why
// the backdrop appeared with no sidebar on mobile. Removed here to leave
// a single source of truth in library.html.
